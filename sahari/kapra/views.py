...
def section_listings(request, head_section_id):
    section_list = Section.objects.filter(headsection__id=head_section_id)
    context = {'section_list':section_list, 'head_section_id':head_section_id }
    return render(request, 'kapra/section_list.html', context)

def employee_listings(request, head_section_id, section_id):
    employee_list = Employee.objects.filter(section__id=section_id)
